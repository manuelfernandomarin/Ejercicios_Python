...
if __name__ == '__main__':
   n = int(input().strip())
   # Evaluation of n parameter if meet resstriction: 0 < n <= 100
   if n < 0 or n > 100:
       print('The number n of elements of matrix  ',n,' is out limit')
       sys.exit('input data out of range: 0 < n <= 100')

   arr = list(map(int, input().rstrip().split()))
   # Evaluate if array called arr elements are into the limits of range
   for k in range(n):
       if arr[k] < -100 or arr[k] > 100:
          print("The element a[i] of array ",arr[k], " is out of range")
          sys.exit('input data out of range: -100 < a[i] <= 100')
       else: pass
