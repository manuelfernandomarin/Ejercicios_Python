#-------------------------------------------------------------------------------
# Name: Maximize It numpy       module1
# Purpose: You are given a function f(x) = X^2. You are also given K lists.
...
aa = [(lambda p : p[1:],ll)]
print ('aa: ', aa) 
import numpy as np # invoque numpy library
# convert list aa to array using numpy function array, then obtain square values
bb = np.array(aa) 
print (type(bb),'bb: ',bb)
bb1 = np.linalg.matrix_power(bb, 2)
# Convert arry to list using function bb.tolist()
aaa = bb1.tolist()
# The print func,in lines 40-42 is no required is used for improve output
print(36*"*")
print(12*' ' + 'ouput:' + 12*' ')
print('')
print(max([sum(map(lambda x:x,i))%m for i in product(*aaa)]))
